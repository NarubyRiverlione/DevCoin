...
*Setup instructions for a Gitian build of Devcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Devcoin
...
- [Building Devcoin Core](#building-devcoin-core)
...
Clone the git repositories for devcoin and Gitian.
...
in the devcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Devcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the devcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/devcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/NarubyRiverlione/devcoin
...
For example:
```bash
URL=https://github.com/thrasher-/devcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit devcoin=${COMMIT} --url devcoin=${URL} ../devcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit devcoin=${COMMIT} --url devcoin=${URL} ../devcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit devcoin=${COMMIT} --url devcoin=${URL} ../devcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the devcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../devcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/devcoin
SIGPATH=/some/root/path/devcoin-detached-sigs

./bin/gbuild --url devcoin=${BTCPATH},signature=${SIGPATH} ../devcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.dev` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/devcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/devcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/devcoin-osx-build.assert
```
...
[gitian.sigs.dev](https://github.com/NarubyRiverlione/gitian.sigs.dev/) repository, or if that's not possible create a pull
